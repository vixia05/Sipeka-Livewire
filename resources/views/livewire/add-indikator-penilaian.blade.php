                ...
                @foreach ($subIndikators as $index => $subIndikator)
                    <div class="flex flex-row gap-4 pb-6 border-b-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="subIndikator" type="text" class="block mt-1 w-fit" wire:key='subIndikators-{{ $index }}'
                        ...
                        <div class="flex flex-col w-full gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="maxSkor" type="number" class="block w-full mt-1" min="0" wire:key='maxPoin-{{ $index }}'
                                    wire:model='maxPoin.{{ $index }}' required />
                            </div>

                            @foreach ($formEvaluasi as $index => $eva)
                                {{-- <!-- Range --> --}}
                                <div class="flex gap-4">
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="startRange" type="number" class="block w-full mt-1"
                                            wire:model='start.sub{{ $loop->parent->index }}{{ $loop->index }}' wire:key='start-{{ $index }}'
                                            min="0" required />
                                    </div>
                                    ...
                                    <div>
                                        <x-text-input id="endRange" type="number" class="block w-full mt-6"
                                            wire:model='end.sub{{ $loop->parent->index }}{{ $loop->index }}' wire:key='end-{{ $index }}'
                                            min="0" required />
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-2 mt-6">
                                        <button type="button"
                                            wire:click.prevent='addEvaluasi({{ $loop->parent->index }}{{ $loop->index }})'
                                        ...
                                        <button type="button"
                                            wire:click.prevent='deleteEvaluasi({{ $index }})'
                    ...
                    <div class="flex flex-row justify-end gap-2 mt-6">
                        ...
                        @endif
                        @if ($loop->last)
                        <button type="button" wire:click.prevent='addIndikator()'
                            class="px-4 py-1 font-semibold text-green-100 transition duration-150 ease-in-out bg-green-500 rounded shadow-md w-fit shadow-green-500/40 brightness-110 hover:brightness-100">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
